...
class SFTDataset(Dataset):
    # 读取一个数据集下的数据
    # 
    def __init__(self, path, tokenizer, max_seq_length, noise_tp = "cut", is_train = True, type = "NER", rate=1.0):
        self.type = type
        ...
        self.example = self.example[:int(rate * len(self.example))]
        # if type == "NER":
    ...
    def __getitem__(self, index):
        ...
        tokenized_in = model_inputs["input_ids"] + [self.eos_token_id]
        tokenized_pmt = pmt["input_ids"] + [self.eos_token_id]
        # tokenized_pmt = tokenized_in
        tokenized_input = [tokenized_in[0]] + [-200, -200, -200, -200] + tokenized_in[1:]  ##pre pad
        # tokenized_input = tokenized_in[:-2] + [-200, -200, -200, -200] + [tokenized_in[-1]]
        # tokenized_input = [tokenized_in[0]] + tokenized_in[1:]
        ...
        labels = [-100] * len(tokenized_input)+ tokenized_label  
        input_ids = input_ids[:self.max_len]
        # test_input_ids = tokenized_pmt[:self.max_len]
        test_input_ids = tokenized_input[:self.max_len]  # 加-200
        labels = labels[:self.max_len]
        p_pmt = tokenized_pmt[:self.max_len]
        ...
        if self.is_train:
            ...
            assert len(input_ids) == len(attention_mask) == len(labels) == len(target_mask)
...
class SFTDataset_all(SFTDataset):
    # 读取一个文件夹下所有数据集的数据
    def __init__(self, path, tokenizer, max_seq_length, noise_tp = "cut", is_train = True, type = "NER", is_random = True):
        self.type = type
        self.tokenizer = tokenizer
        self.bos_token_id = tokenizer.bos_token_id
        self.eos_token_id = tokenizer.eos_token_id
