...
class AD_Dropout(torch.nn.Module):
    def __init__(self, p):
    ...
    def create_mask(self, A, p):
        # 将矩阵 A 展开成一维
        flattened_A = A.view(-1)
        # 计算 p% 的位置数量
        k = int(p * flattened_A.numel() / 100)
        # 找到值最大的 p% 的位置
        _, indices = torch.topk(flattened_A, k)
        # 创建一个全是1的 mask 矩阵
        mask = torch.ones_like(flattened_A)
        # 将最大的 p% 的位置设置为0
        mask[indices] = 0
        # 将 mask 矩阵重新形状为 A 的形状
        mask = mask.view(A.shape)
        return mask
    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                noise_in = noise if idx == (len(self.layers) - 1) else None
