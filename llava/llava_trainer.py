...
class LLaVATrainer(Trainer):
    ...
    ignore_keys: Optional[List[str]] = None,
        ...
        with torch.no_grad():
            if is_sagemaker_mp_enabled():
                ...
                if has_labels or loss_without_labels:  #原版
                # if loss_without_labels:
    ...
    def eval_gen(self, model, inputs, args, return_outputs = True, generation_config = None):
        
        # generated_tokens = model.generate(
        #                             inputs=inputs["input_ids"], images = inputs["images"], max_new_tokens=generation_config.max_new_tokens, do_sample=True,
        ...
        #                             eos_token_id=generation_config.eos_token_id#,attention_mask = inputs["test_mask_batch"]
        #                         )
        
        generated_tokens = model.generate(
                                    input_ids=inputs["test_ids"], max_new_tokens=generation_config.max_new_tokens, do_sample=True,
                                    top_p=generation_config.top_p, temperature=generation_config.temperature, repetition_penalty=generation_config.repetition_penalty,
                                    eos_token_id=generation_config.eos_token_id#,attention_mask = inputs["test_mask_batch"]
                                )
        
        loss = None
        return (loss, generated_tokens, inputs["labels"]) if return_outputs else loss
